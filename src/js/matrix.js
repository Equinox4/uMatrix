...
µMatrix.Matrix = (() => {
	...
	const extractFirstPartyDesDomain = function (srcHostname, desHostname) {
		...
		let µmuri = µm.URI;
		let srcDomain = µmuri.domainFromHostname(srcHostname) || srcHostname;
		let desDomain = µmuri.domainFromHostname(desHostname) || desHostname;
	...
	Matrix.prototype.evaluateSwitch = function (switchName, srcHostname) {
		...
		let bitOffset = switchBitOffsets.get(switchName);
		if (bitOffset === undefined) {
			return 0;
		}
